...
module.exports = {
  ...
  async execute(ctx) {
    ...
    let target = sender;
    // Lógica para detectar si respondió a alguien o mencionó a alguien
    ...
    const xp = user.xp || 0;
    const level = user.level || 0; // Usamos el nivel guardado

    // 🔥 Regla: 10,000 XP por nivel
    ...
    const displayUser = target === sender ? `👤 ${pushName}` : `👤 @${number}`;
